...
class viga:
    ...
    def construir_beam_truss(self):
        ...
        faces= [[(0, 1), (2, 4)], [(2, 4), (3, 5)], [(3, 5), (6, 7)], [(0, 1), (6, 7)]] #face1,face2,face3,face4
        ...
        for face in faces:
            ...
            q1=V[face[1][0]]
            q2=V[face[1][1]]
            vv=q2-q1/self.num_crosses
            m = q1-p1+vv #vv / np.linalg.norm(vv)
            n=vv-q1+p1
        ...
        for edge in ab_edges:
            ...
            for k in range(self.num_crosses):
                current_point = start_vertex[0] + k*((end_vertex[0] - start_vertex[0])/self.num_crosses)

                # Calculate cross-section lines in the Y and Z directions
                if self.width > 0:
                    y_start = start_vertex[1] - self.width / 2
                    y_end = start_vertex[1] + self.width / 2
                    self.acad.model.Add3Dpoly(
                        aDouble(current_point, y_start, start_vertex[2],
                                current_point, y_end, start_vertex[2]))
                    #print("Para la arista : ",edge, "el ancho es: ", self.width)
                    #print([current_point, y_start, start_vertex[2],
                    #            current_point, y_end, start_vertex[2]])
                if self.height > 0:
                    z_start = start_vertex[2] - self.height / 2
                    z_end = start_vertex[2] + self.height / 2
                    self.acad.model.Add3Dpoly(
                        aDouble(current_point, start_vertex[1], z_start,
                                current_point, start_vertex[1], z_end))
                    #print("Para la arista : ",edge, "La altura es: ",self.height)
                    #print([current_point, start_vertex[1], z_start,
                    #            current_point, start_vertex[1], z_end])
def main():
