...
class viga:
    def __init__(self, acad, length, width, height, num_crosses,nivel_de_conexion,punto_inicio_portico,punto_fin_portico,largo_top):
        self.acad = acad
        self.length = length
        self.width = width
        self.height = height
        ...
        self.punto_inicio_portico=punto_inicio_portico
        self.punto_fin_portico=punto_fin_portico
        self.largo_top=largo_top
    ...
    def dibujar(self):
        ...
        V_list_modulus=self.vector_modulus_unit(V)[0]
        V_list_unit_vectors=self.vector_modulus_unit(V)[1]
        
        V=unitario(pf-pi) x V_list_modulus
    ...
    def vector_modulus_unit(list_of_vector):
        list_of_modulus=[]
        list_of_unit_vector=[]

        for vector in list_of_vector:
            modulus = (vector[0]**2 + vector[1]**2 + vector[2]**2) ** 0.5
            if modulus < 1e-10: 
                raise ValueError("Cannot compute unit vector of a zero vector.")
            unit_vector = (vector[0] / modulus, vector[1] / modulus, vector[2] / modulus)
            list_of_modulus.append(modulus)
            list_of_unit_vector.append(unit_vector)
        return list_of_modulus, list_of_unit_vector
...
def main():
    ...
    viga1 = viga(acad, length, width, height, num_crosses_box,altura_de_nivel_de_conexion,punto_inicio_portico,punto_fin_portico,largo_top)
    viga1.dibujar()
