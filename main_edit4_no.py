...
class viga:
    ...
    def dibujar(self):
        ...
        self._agregar_cruces()
    
    def _agregar_cruces(self):
        V = self.parametrizacion()[0]
        edges = self.parametrizacion()[1]
        ...
        for edge in ab_edges:
            ...
            direction = end_vertex[0] - start_vertex[0]

            # Calculate step size based on the number of crosses
            step = direction / self.num_crosses

            for k in range(self.num_crosses):
                current_point = start_vertex[0] + step * k
